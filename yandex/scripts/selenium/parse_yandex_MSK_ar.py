...
def parse_item_list(soup, add):
    ...
    for row in rows:
        ...
        price_start = cells[2].find("div", class_='OffersArchiveSearchOffers__price').span.text.replace("₽ / мес.","").replace(' ','').replace(",",'.').replace("₽",'')

        m_price_start = cells[2].find("div", class_='OffersArchiveSearchOffers__extra-info').span.text.replace(" ", '').replace(' ','').replace("₽",'').replace(",",'.')

        price_end = cells[3].find("div", class_='OffersArchiveSearchOffers__price').span.text.replace("₽ / мес.","").replace(' ','').replace(",",'.').replace("₽",'')
...
def main():
    ...
    time.sleep(15)
    type = 1 # 0- покупка 1- продажа
    file = "database_MSK.csv"
    urls, addresses = get_links(file, type)
    items = []
    gets = 0

    filename = "yandexMSKarSelenium"
